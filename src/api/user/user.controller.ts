...
export class UserController {
  ...
  async checkNickName(@Param() nickNameDto: NickNameDto) {
    ...
    return new SuccessResponseDTO(
      await this.userService.findUserNickName(nickName),
    );
  }
  ...
  async updateProfile(
    ...
    const { uuid } = req.user;
    return new SuccessResponseDTO(
      await this.userService.updateUserProfile(nickName, uuid, file),
    );
  }
  ...
  async getSubmitShop(@Req() req) {
    const { uuid } = req.user;
    return new SuccessResponseDTO(
      await this.userService.findSubmitRecord(uuid),
    );
  }
